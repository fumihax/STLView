...
//	DEBUG_INFO("Start CBREPView::OnInitialUpdate()");
	if (Solid==NULL) return;
//	facetno = Solid->contours.size();
	facetno = Solid->facetno;
//	DEBUG_INFO("facetno = %d", facetno);
	if (facetno==0) return;
	...
	sizeFac = Min(sizeFac, (float)(1./msize));
	...
	if (!rslt) {
		MessageBox(_T("BREPview::OnInitialUpdate: Dx9デバイスの取得に失敗"));
...
//	mVecDir = D3DXVECTOR3(1.0, 2.0, 1.0);
	...
	//D3DXMatrixPerspectiveFovLH(&matProj, (float)(60.0f * PI / 180.0f), 1.0f, 0.1f, (float)(10 * msize * sizeFac));
	D3DXMatrixPerspectiveFovLH(&matProj, (float)(PI / 3.0), 1.0f, 0.1f, (float)(10 * msize * sizeFac));
...
{
	if (Xabs(mWheelAc)<0.5) mWheelAc = (float)(Sign(mWheelAc)*0.5); // 0.5 以下には接近しない
...
//	hr = lpD3DDevice->Clear(0, NULL, D3DCLEAR_TARGET|D3DCLEAR_ZBUFFER, D3DCOLOR_XRGB(10,10,120), 1.0f, 0);
	...
	if (FAILED(hr)) { DEBUG_MODE PRINT_MESG("CBREPView::ExecRender: 画面クリア失敗\n"); return;}

	// シーンの描画開始
	hr = lpD3DDevice->BeginScene();	
	if (FAILED(hr)) { DEBUG_MODE PRINT_MESG("CBREPView::ExecRender: シーンの開始失敗\n"); return;}
	
	if (solided) {
		hr = lpD3DDevice->SetStreamSource(0, vb, 0, sizeof(SBREP_VERTEX));
		if (FAILED(hr)) { DEBUG_MODE PRINT_MESG("CBREPView::ExecRender: ストリームの設定失敗 BREP_SOLID\n"); lpD3DDevice->EndScene(); return;}
		hr = Dx9DrawPrimitive(lpD3DDevice, D3DPT_TRIANGLELIST, 3, facetno);
		if (FAILED(hr)) { DEBUG_MODE PRINT_MESG("CBREPView::ExecRender: プリミティブ描画失敗 BREP_SOLID\n"); lpD3DDevice->EndScene(); return;}
	}

	if (wired) {
		hr = lpD3DDevice->SetStreamSource(0, lb, 0, sizeof(SBREP_VERTEX));
		if (FAILED(hr)) { DEBUG_MODE PRINT_MESG("CBREPView::ExecRender: ストリームの設定失敗 WIRE\n"); lpD3DDevice->EndScene(); return;}
		hr = Dx9DrawPrimitive(lpD3DDevice, D3DPT_LINELIST, 2, facetno*3);
		if (FAILED(hr)) { DEBUG_MODE PRINT_MESG("CBREPView::ExecRender: プリミティブ描画失敗 WIRE\n"); lpD3DDevice->EndScene(); return;}
	}

	hr = lpD3DDevice->EndScene();	// シーンの描画終了
	if (FAILED(hr)) { DEBUG_MODE PRINT_MESG("CBREPView::ExecRender: シーンの終了失敗\n"); return;}
	// シーンの描画終了
...
//		DEBUG_MODE PRINT_MESG("CBREPView::ExecRender: デバイスリセット中!!"); 
		...
		if (FAILED(hr)) {
			MessageBox(_T("CBREPView::ExecRender: DirextX9のデバイスを復元できません!\n!"));
...
BOOL   CBREPView::PrepareVB()
{
	...
	if (vb==NULL || lb==NULL) {			// メモリ確保失敗
		...
		ReleaseVB();
		MessageBox(_T("CBREPView::PrepareVB: 頂点バッファの確保に失敗"), _T("エラー"));
 		return  FALSE;
	}
